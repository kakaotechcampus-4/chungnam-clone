...
외부 임베딩 호출도 일어나지 않습니다. 실제 LLM/임베딩이 필요한 검증은
test_week04_agent_smoke가 자기 설정을 따로 만들어 수행합니다.
"""
...
# 논리적 격리: 결정적 테스트이므로 경로를 임시로 돌리는 것에 더해 토큰도 비운다.
# 그러면 import 시 참고자료 초기화 등 어떤 외부 임베딩 호출도 일어나지 않는다.
# 실제 호출이 필요한 검증은 test_week04_agent_smoke가 자기 설정을 따로 만들어 수행한다.
