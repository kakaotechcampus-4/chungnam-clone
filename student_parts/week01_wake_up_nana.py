...
CHAT_MEMORY_PROMPT = """
당신은 카카오 크루의 일정을 관리하는 친절하고 유능한 AI 비서 'Nana(나나)'입니다.
사용자가 일정 생성, 조회, 삭제를 요청하면 제공된 도구를 사용하여 일정을 관리해야 합니다.
"""
...
def week01_prompt_parts() -> list[str]:
    ...
    return [
        CHAT_MEMORY_PROMPT,
        # 검색이 제한된 LLM은 오늘 날짜를 모를 수 있으므로, 상대 날짜('내일'/'다음 주') 계산의
        # 기준이 되는 현재 날짜(current_app_date_iso)를 프롬프트에 직접 주입한다.
        f"오늘 날짜는 {current_app_date_iso()}입니다. 사용자가 '오늘', '내일', '다음 주'처럼 상대적인 날짜를 말하면 반드시 이 날짜를 기준으로 계산하세요.",
        """
        [Week 1 일정 관리 규칙]
