...
CHAT_MEMORY_PROMPT = "이전 대화 내용을 기억하고 일관성 있게 답변하세요."
...
def personal_delete_schedule(schedule_id: str) -> str:
    ...
    front_len = len(PERSONAL_SCHEDULES)
    
    # PERSONAL_SCHEDULES.pop(_current_session_schedules()) <- pop은 인덱스(숫자)만 가능한데 _current_session_schedules()는 리스트를 반환해서 들어갈 수 없다
    # 1. 현재 세션 + schedule_id 조건으로 삭제 대상 찾기
    # 2. PERSONAL_SCHEDULES에서 해당 항목만 빼고 나머지로 교체
    PERSONAL_SCHEDULES[:] = [
        s for s in PERSONAL_SCHEDULES
        if not (s["id"] == schedule_id and _schedule_scope(s) == current_session_scope())
    ]
    back_len = len(PERSONAL_SCHEDULES)
    deleted = back_len < front_len
    return _json({
        "ok": True,
