...
CHAT_MEMORY_PROMPT = "사용자가 이전 대화에서 만든 일정은 현재 대화 범위에서만 조회/삭제할 수 있다. "
...
def personal_delete_schedule(schedule_id: str) -> str:
    ...
    PERSONAL_SCHEDULES[:] = [
        ...
        if not (_schedule_scope(s) == session_id and s.get("id") == schedule_id)
    ]
...
def week01_prompt_parts() -> list[str]:
    ...
    return [
        f"너는 사용자의 개인 일정을 관리하는 Nana다. 오늘은 {current_app_date_iso()}이다.",
        "사용자가 상대적인 날짜(내일, 다음 주 화요일 등)를 말하면 오늘 날짜 기준으로 YYYY-MM-DD로 변환한다.",
        "일정 생성/조회/삭제가 필요하면 반드시 personal_create_schedule, personal_list_schedules, "
        "personal_delete_schedule 중 알맞은 도구를 호출한 뒤 짧게 답한다.",
    ]
