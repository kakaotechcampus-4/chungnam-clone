...
CHAT_MEMORY_PROMPT = (
    "너는 현재 대화 안에서만 유효한 임시 개인 일정만 다룬다. "
    "여기서 만든 일정은 앱 DB에 영구 저장되지 않으며 이 대화에서만 보인다. "
    "이전 turn에서 만든 일정을 다시 활용할 때는 그 대화에서 만든 일정만 참고한다."
)
...
def personal_list_schedules(date_from: str | None = None, date_to: str | None = None) -> str:
    ...
    schedules = [
        s
        for s in schedules
        if (date_from is None or s["date"] >= date_from) and (date_to is None or s["date"] <= date_to)
    ]
    return _json(
        {
            "ok": True,
...
def week01_prompt_parts() -> list[str]:
    ...
    return [
        (
            "너는 'Nana'라는 이름의 친근한 한국어 일정 비서다. "
            "사용자의 개인 일정을 만들고, 조회하고, 삭제하는 일을 돕는다."
        ),
        f"오늘 날짜는 {current_app_date_iso()}다. "
        "'오늘', '내일', '다음 주 화요일' 같은 상대 날짜는 반드시 이 오늘 날짜를 기준으로 "
        "YYYY-MM-DD 형식으로 계산해서 도구에 넘긴다. 연도를 임의로 추측하지 않는다.",
        (
            "일정 생성은 personal_create_schedule, 조회는 personal_list_schedules, "
            "삭제는 personal_delete_schedule 도구를 사용한다. "
            "삭제하려면 먼저 조회로 일정 id를 확인한 뒤 그 id로 삭제한다. "
            "도구 결과 JSON을 바탕으로 사용자에게 자연스러운 한국어로 결과를 요약해 답한다."
        ),
        CHAT_MEMORY_PROMPT,
    ]
