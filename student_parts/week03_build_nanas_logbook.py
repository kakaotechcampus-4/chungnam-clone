...
SQLITE_MEMORY_PROMPT = (
    "너의 일정/할 일/알림은 앱 SQLite DB에 영구 저장된다. "
    "이 기록장은 대화가 끝나거나 앱을 다시 시작해도 사라지지 않으므로, "
    "과거에 저장한 항목을 물으면 기억에 의존하지 말고 반드시 조회 tool로 DB에서 확인해 답한다."
)

# TODO: 자연어 구조화 → SQLite 저장과 조회/수정/삭제 tool 호출 순서를 안내하는 규칙을 작성하세요.
WEEK03_TOOL_CALL_PROMPT = (
    "일정/할 일/알림 저장 요청을 받으면 먼저 extract_schedule_request(query=사용자 요청)로 "
    "자연어를 구조화한 뒤, 그 결과 필드를 save_structured_request 인자로 그대로 넘겨 저장한다. "
    "저장된 요청 확인은 list_saved_requests(단건은 get_saved_request), "
    "저장된 일정 조회는 personal_list_saved_schedules를 호출한다. "
    "상대 날짜는 오늘 날짜를 기준으로 YYYY-MM-DD로 변환한다."
)
...
def week03_tools() -> list[Any]:
    ...
    _LEGACY_EXCLUDE = {"personal_list_schedules", "personal_delete_schedule"}
    base_tools = [
        personal_create_schedule if _tool_name(item) == "personal_create_schedule" else item
        for item in week01_tools()
        if _tool_name(item) not in _LEGACY_EXCLUDE
    ]
    return [
        *base_tools,
        extract_schedule_request,
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        "Week 2에서 구조화한 요청은 그대로 두지 말고, Week 3 SQLite 저장 흐름으로 이어 기록장에 남긴다.",
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        # TODO: 현재 날짜, Week 3 tool 선택 기준, 이번 주차의 범위를 설명하는 agent 지시를 추가하세요.
        f"오늘 날짜는 {current_app_date_iso()}이다. "
        "저장과 조회는 위 Week 3 tool만 사용하며, 이번 주차 범위는 구조화 결과의 저장과 조회다.",
    ]
