...
SQLITE_MEMORY_PROMPT = (
    "Week 3부터 개인 일정/할 일/알림은 현재 대화의 임시 메모리가 아니라 앱 SQLite DB에 저장됩니다. "
    "저장된 기록은 새 대화를 시작하거나 앱을 다시 켜도 그대로 남아 있으므로, 이전에 저장한 일정을 "
    "확인할 때는 현재 대화에서 기억나는 값을 추측하지 말고 personal_list_saved_schedules, "
    "list_saved_requests, get_saved_request로 SQLite에서 직접 조회합니다."
)

WEEK03_TOOL_CALL_PROMPT = (
    "자연어 일정/할 일/알림 요청은 먼저 extract_schedule_request로 구조화한 뒤, 그 결과의 "
    "structured_request 필드 값을 그대로 save_structured_request 인자로 전달해 SQLite에 저장합니다. "
    "일정 조회 요청에는 personal_list_saved_schedules 또는 list_saved_requests/get_saved_request를 사용합니다. "
    "저장된 일정을 수정하거나 삭제하기 전에는 먼저 personal_list_saved_schedules로 대상 schedule_id를 "
    "확인한 뒤 personal_update_saved_schedule 또는 personal_delete_saved_schedules를 호출합니다. "
    "개인 일정을 새로 만드는 요청에는 personal_create_schedule을 호출합니다. 이 tool은 Week 1 임시 일정 "
    "생성과 SQLite 저장을 함께 수행합니다."
)
...
class SaveStructuredRequestInput(StructuredRequest):
    ...
    def unwrap_legacy_payload(cls, value: Any) -> Any:
        ...
        if isinstance(value, dict):
            nested = value.get("structured_request")
    ...
    *,
    ...
    if not delete_all and not any([schedule_ids, date, title, start_time, time_unspecified]):
        return tool_result(
            ...
            ok=False,
            error="삭제 조건이 없습니다. schedule_ids 또는 날짜/제목/시간 필터를 지정하거나 delete_all=True를 사용하세요.",
            deleted_count=0,
            filters=filters,
            deleted=[],
        )
...
def personal_create_schedule(
    ...
    created_schedule = created_payload.get("created_schedule", {})
...
def save_structured_request(
    ...
    result = _store().save_structured_request({key: val for key, val in payload.items() if val is not None})
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        f"오늘 날짜는 {current_app_date_iso()}입니다. 당신은 이제 Week 3 기록장 agent로, 구조화된 요청을 "
        "SQLite에 저장하고 조회/수정/삭제하는 것까지 담당합니다. RAG 검색이나 여러 사람 일정 조율은 "
        "이번 주차 범위가 아니므로 다루지 않습니다.",
    ]
