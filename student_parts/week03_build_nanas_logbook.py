...
WEEK03_TOOL_CALL_PROMPT = (
    "일정/할 일/알림을 저장해야 하면 먼저 extract_schedule_request(query=사용자 원문)를 호출해 자연어를 구조화하고, "
    "그 결과의 structured_request 필드 값을 그대로 save_structured_request 인자로 전달해 저장한다. "
    "저장된 일정을 수정/삭제하기 전에는 먼저 personal_list_saved_schedules로 후보 schedule_id를 확인한다. "
    "수정은 personal_update_saved_schedule, 삭제는 personal_delete_saved_schedules를 사용하며, "
    "schedule_ids 또는 명확한 필터(date/title/start_time/delete_all) 없이 삭제를 호출하지 않는다."
)
        ...
        if isinstance(value, dict):
            ...
            if isinstance(inner, dict):
                ...
                if "source_schedule_id" in value:
                    merged.setdefault("source_schedule_id", value["source_schedule_id"])
                return merged
...
def personal_create_schedule(
    ...
    schedule = json.loads(created_raw)["created_schedule"]
...
def save_structured_request(
    ...
    payload = {
        "kind": kind, "title": title, "date": date, "start_time": start_time,
        "end_time": end_time, "members": members or [], "priority": priority,
...
def delete_saved_schedules_dict(
    ...
    return _delete_saved_schedules(
        store=store, schedule_ids=schedule_ids, date=date, title=title,
...
def personal_update_saved_schedule(
    ...
    result = _store().update_schedule(
        schedule_id, title=title, date=date, start_time=start_time,
        end_time=end_time, attendees=attendees,
    )
    if result is None:
        return json_payload(tool_result(
            "personal_update_saved_schedule", ok=False,
            error=f"schedule_id {schedule_id}를 찾을 수 없습니다.",
        ))
    return json_payload(tool_result(
        "personal_update_saved_schedule",
        updated_schedule=result["schedule"], shared_sync=result["shared_sync"],
    ))
...
def personal_delete_saved_schedules(
    ...
    result = _delete_saved_schedules(
        store=_store(), schedule_ids=schedule_ids, date=date, title=title,
        start_time=start_time, time_unspecified=time_unspecified, delete_all=delete_all,
    )
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        f"오늘은 {current_app_date_iso()}이다. 일정 저장/조회/수정/삭제는 이번 주차의 SQLite tool로만 처리하고, "
        "RAG 검색이나 외부 멤버 일정 조율은 이번 주차 범위가 아니다.",
    ]
