...
SQLITE_MEMORY_PROMPT = (
    "Week 3부터 일정/할 일/알림은 현재 대화가 끝나도 사라지지 않고 SQLite 앱 DB에 영구 저장된다. "
    "list_saved_requests, get_saved_request, personal_list_saved_schedules로 새 대화나 재시작 이후에도 "
    "이전에 저장한 기록을 조회할 수 있으니, 사용자가 과거에 저장한 내용을 물으면 먼저 조회 tool부터 사용해."
)
...
WEEK03_TOOL_CALL_PROMPT = (
    ...
    "삭제는 schedule_ids나 명확한 필터가 있을 때만 수행하고, 조건이 불명확하면 조건 없이 지우지 말고 사용자에게 먼저 확인해."
)
...
class SaveStructuredRequestInput(StructuredRequest):
    ...
    def unwrap_legacy_payload(cls, value: Any) -> Any:
        ...
        if isinstance(value, dict):
            wrapper_key = "structured_request" if isinstance(value.get("structured_request"), dict) else (
                "payload" if isinstance(value.get("payload"), dict) else None
            )
            if wrapper_key is not None:
                inner = dict(value[wrapper_key])
                for key in ("source_schedule_id",):
                    if key in value and key not in inner:
                        inner[key] = value[key]
                return inner
        return value
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        f"너는 Week 3 기록장 agent야. 현재 날짜는 {current_app_date_iso()}이야.",
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        # TODO: 현재 날짜, Week 3 tool 선택 기준, 이번 주차의 범위를 설명하는 agent 지시를 추가하세요.
        "이번 주차(Week 3)의 범위는 개인 일정/할 일/알림의 SQLite 저장·조회·수정·삭제까지다. "
        "여러 사람 일정 조율이나 RAG 검색은 이번 주차 범위가 아니다.",
    ]
