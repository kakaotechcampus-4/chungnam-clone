...
SQLITE_MEMORY_PROMPT = textwrap.dedent("""
    ...
    사용자가 '내 일정 뭐 있어?', '아까 저장한 거 뭐였지?' 처럼
    기존 일정/할 일/알림의 존재나 내용을 묻는 말이면(꼭 '저장된'이라는 단어가 없어도),
    네 기억에 의존하지 말고 반드시 조회 tool로 DB를 직접 확인한 뒤 답해라.
    일정인지 할 일/알림인지에 따라 어떤 tool을 써야 하는지는 아래 기준을 따라라.
""").strip()

WEEK03_TOOL_CALL_PROMPT = textwrap.dedent("""
    1. 자연어 요청은 extract_schedule_request로 구조화한 뒤, 그 결과 필드 값을
       save_structured_request 인자로 그대로 전달해 저장한다(원문을 구조화 없이 바로 넘기지 않는다).
    2. personal_create_schedule은 Week 1 호환용으로 남아 있을 뿐이니, 새 개인 일정 저장은
       1번의 2단계 흐름을 우선 쓰고 같은 요청에 두 tool을 동시에 호출하지 않는다.
    3. 일정 수정/삭제 전에는 먼저 personal_list_saved_schedules로 후보와 schedule_id를
       확인한 뒤, personal_update_saved_schedule 또는 personal_delete_saved_schedules에
       schedule_ids(또는 명시적 date/title/start_time 필터)를 전달한다.
    4. 조건 없는 삭제는 실행하지 않는다. delete_all은 사용자가 "전부 삭제해줘"처럼
       명시적으로 전체 삭제를 요청했을 때만 쓴다.
""").strip()
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    raise RuntimeError(f"예상치 못한 저장 입력 타입: {type(value)}")
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        "이 tool은 기본적으로 personal_schedule만 찾으니, 그룹 일정을 조회할 때는 "
