...
SQLITE_MEMORY_PROMPT = (
    "일정/할 일/알림 정보는 SQLite 데이터베이스에 저장된다. "
    "SQLite에 저장된 정보는 앱 재실행이나 새 대화 이후에도 유지된다. "
    "저장 여부나 기존 일정 목록을 확인할 때는 SQLite 도구 결과를 기준으로 판단한다. "
    "저장 도구의 성공 결과를 확인하기 전에는 저장됐다고 단정하지 않는다. "
    "사용자가 저장된 일정이나 요청을 묻는 경우 기억에 의존하지 않고 SQLite 조회 도구를 호출한다. "
    "대화 기록이 없거나 새로운 대화여도 SQLite에 저장된 일정은 조회할 수 있다. "
    "조회 결과가 비어있으면 일정이 없다고 안내하며 존재하지 않는 일정을 추측하지 않는다. "
    "일정의 존재 여부, ID, 날짜, 시간, 제목은 SQLite 도구 반환값을 기준으로 판단한다."
)

WEEK03_TOOL_CALL_PROMPT = (
    "[저장 요청] - 사용자가 저장을 요청하면 먼저 구조화하고 구조화 결과의 각 필드를 저장 도구에 전달한다. "
    "[조회 요청] - 조회 요청은 바로 SQLite DB에서 조회한다. "
    "[수정 요청] - 수정 요청은 먼저 저장된 일정을 조회해 수정 대상을 확인한다. 대상이 명확하면 바로 수정하고, 여러 후보가 있거나 모호하면 사용자에게 확인받는다. "
    "[삭제 요청] - 삭제 요청은 먼저 저장된 일정을 조회해 삭제 대상을 확인한다. 대상이 명확하면 바로 삭제하고, 여러 후보가 있거나 모호하면 사용자에게 확인받는다. "
    "수정, 삭제시 날짜나 제목 보다 schedule_id를 우선 사용한다. "
    "수정이나 삭제 전에 조회한 결과를 그대로 활용해야 한다. 임의로 추측하지 않는다. "
    "저장/수정/삭제 여부 판단은 도구 반환값을 기준으로 답한다."
)
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    raise TypeError(f"지원하지 않는 저장 입력 타입입니다: {type(value)!r}")
...
def save_structured_request_payload(
    ...
    *,
    ...
    clean_payload = {
        key: value
        for key, value in save_input.model_dump().items()
...
def personal_create_schedule(
    ...
    if not created_result.get("ok", False):
        return json_payload(created_result)
...
def save_structured_request(
    ...
    store = _store()
    store_result = store.save_structured_request(clean_payload)
...
def personal_update_saved_schedule(
    ...
    store = _store()
    update_result = store.update_schedule(
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        Week 3에서는 사용자의 자연어 요청을 구조화한 뒤 SQLite에 저장하고 조회할 수 있다.
        일정/할 일/알림 저장 요청은 먼저 구조화한 후 save_structured_request를 사용해라.
        저장된 일정 조회는 personal_list_saved_schedules를 사용해라.
        저장된 구조화 요청 조회는 list_saved_requests 또는 get_saved_request를 사용해라.
        사용자의 요청에 맞는 도구를 선택하고, 도구 결과를 기준으로 응답해라.
        """
        )
    ]
