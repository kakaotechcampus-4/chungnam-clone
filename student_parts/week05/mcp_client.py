...
_LOCAL_MCP_TOOLS_CACHE: dict[str, list[Any]] = {}
...
def clear_local_mcp_tools_cache() -> None:
    _LOCAL_MCP_TOOLS_CACHE.clear()
...
async def load_local_mcp_tools(
    ...
    / "student_parts"
    / "week05"
    / "extended_mcp_server.py"
)
    env = os.environ.copy()
    selected_db_path = (
        db_path
        or env.get("KANANA_EXTERNAL_DB_PATH")
        or CONFIG.external_db_path
    )

    cache_key = str(Path(selected_db_path).resolve())
    cached_tools = _LOCAL_MCP_TOOLS_CACHE.get(cache_key)
    ...
    _LOCAL_MCP_TOOLS_CACHE[cache_key] = tools
    return tools
