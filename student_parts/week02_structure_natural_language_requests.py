...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="StructuredRequest 목록을 담고, 요청이 하나뿐이어도 list 형태를 유지.")
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜 해석 기준일을 담는다.")
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        # 1. Week 2 agent 역할 + 오늘 날짜 기준
        f"너는 Week 2 요청 구조화(structuring) 담당 agent다. "
        f"오늘 날짜는 {current_app_date_iso()}이다. "
        f"'내일', '다음 주 화요일' 같은 상대 날짜 표현은 이 날짜를 기준으로 실제 날짜(YYYY-MM-DD)로 계산한다.",

        # 2. 자연어 -> StructuredRequest 필드 구조화 지시
        "사용자의 자연어 요청에서 kind(personal_schedule/group_schedule/todo/reminder/unknown), "
        "title, date, start_time, end_time, members, priority, reason을 추출해 StructuredRequest 필드로 채운다. "
        "확실하지 않은 값은 억지로 만들지 말고 None 또는 빈 리스트로 둔다.",

        # 3. Week 1 tool 결과 JSON 재사용 지시 (재호출 금지)
        "personal_create_schedule 같은 Week 1 tool의 실행 결과 JSON을 이미 받았다면, "
        "같은 tool을 다시 호출하지 말고 그 JSON(특히 created_schedule)을 읽어 structured_response 필드를 채운다.",

        # 4. Week 2 범위 제한 명시
        "Week 2에서는 SQLite 저장, RAG 검색, 외부 멤버와의 일정 조율을 수행하지 않는다. "
        "오직 자연어/tool 결과를 구조화한 StructuredRequestBatch만 반환한다.",
