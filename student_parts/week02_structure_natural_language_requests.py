...
class StructuredRequest(BaseModel):
    ...
    priority: str | None = Field(default=None, description="우선순위 (예: 높음/보통/낮음). 확실하지 않으면 None")
    reason: str | None = Field(default=None, description="LLM이 위 필드를 채운 판단 근거")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        ...
        description="구조화된 요청 목록. 요청이 하나뿐이어도 반드시 리스트로 담는다",
    )
    base_date: str = Field(
        default_factory=current_app_date_iso,
        description="상대 날짜 해석 기준일 (YYYY-MM-DD). 오늘 날짜로 자동 설정된다",
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        f"너는 사용자의 자연어 요청과 Week 1 tool 결과를 StructuredRequest로 구조화하는 agent다. "
        f"오늘은 {current_app_date_iso()}이다. 사용자가 상대적인 날짜(내일, 다음 주 화요일 등)를 말하면 "
        "오늘 날짜 기준으로 YYYY-MM-DD로 변환해 date 필드에 채운다.",
        "personal_create_schedule tool 결과 JSON의 created_schedule 필드를 읽어 StructuredRequest를 채운다. "
        "이미 tool을 호출했다면 같은 tool을 다시 호출하지 않는다.",
    ...
    return join_system_prompt(
        [
            *week02_prompt_parts(),
            ...
            "personal_create_schedule tool 결과의 created_schedule을 읽어 "
