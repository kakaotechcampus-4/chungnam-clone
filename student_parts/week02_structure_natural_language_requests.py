...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(description="요청 종류: personal_schedule/group_schedule/todo/reminder/unknown 중 하나, 모르면 unknown")
    ...
    start_time: str | None = Field(default=None, description="HH:MM 형식 시작 시간")
    end_time: str | None = Field(default=None, description="HH:MM 형식 종료 시간")
    ...
    reason: str | None = Field(default=None, description="이렇게 판단한 근거")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="구조화된 요청 목록 (요청이 1개여도 리스트로 담음)")
    # TODO: base_date 필드를 str 타입으로 선언하고 default_factory=current_app_date_iso를 사용하세요.
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜를 해석하는 기준일")
...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    raise RuntimeError(f"예상치 못한 structured output 형태: {type(value)!r}")
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        f"너는 이제 자연어 요청이나 Week 1 tool이 반환한 JSON을 StructuredRequestBatch로 구조화하는 역할도 한다. "
        f"오늘 날짜는 {current_app_date_iso()}이며 상대 날짜는 이 날짜 기준으로 YYYY-MM-DD로 바꾼다.",
        "personal_create_schedule 같은 tool 결과 JSON을 받았다면 같은 tool을 다시 호출하지 말고, "
        "그 안의 created_schedule 값을 읽어 kind/title/date/start_time/end_time/members 등 필드를 채운다.",
        "한 문장에 여러 일정/할 일/알림 의도가 섞여 있으면 requests 리스트에 StructuredRequest를 여러 개 담고, "
        "요청이 하나뿐이어도 requests 리스트 안에 하나를 담아 반환한다.",
