...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(
        description=(
            "요청 종류. personal_schedule(개인 일정), group_schedule(그룹/회의 일정), "
            "todo(할 일), reminder(알림), unknown(분류 불가) 중 하나만 사용한다."
        )
    )
    title: str | None = Field(
        default=None,
        description="일정/할 일/알림의 제목. 확실하지 않으면 None으로 둔다.",
    )
    date: str | None = Field(
        default=None,
        description="일정 날짜. 확실할 때만 YYYY-MM-DD 형식으로 채우고, 모르면 None으로 둔다.",
    )
    start_time: str | None = Field(
        default=None,
        description="시작 시간. 확실할 때만 HH:MM 형식으로 채우고, 모르면 None으로 둔다.",
    )
    end_time: str | None = Field(
        default=None,
        description="종료 시간. 확실할 때만 HH:MM 형식으로 채우고, 모르면 None으로 둔다.",
    )
    members: list[str] = Field(
        default_factory=list,
        description="참석자나 관련 멤버 이름 목록. 언급이 없으면 빈 list로 둔다.",
    )
    priority: str | None = Field(
        default=None,
        description="할 일 우선순위(예: 높음/보통/낮음). 명시가 없으면 None으로 둔다.",
    )
    reason: str | None = Field(
        default=None,
        description="이 요청 종류/필드로 판단한 근거. 필요 없으면 None으로 둔다.",
    )
    original_text: str = Field(
        default="",
        description="구조화 이전의 원문 요청 텍스트를 그대로 보존한다.",
    )
...
class StructuredRequestBatch(BaseModel):
    ...
    base_date: str = Field(
        ...
        description="'오늘/내일/다음 주' 같은 상대 날짜를 해석하는 기준일(YYYY-MM-DD).",
    )
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt(
        [
            *week02_prompt_parts(),
            (
                "너의 최종 답변은 반드시 StructuredRequestBatch 형식이어야 한다. "
                "요청이 하나뿐이어도 requests 목록에 StructuredRequest 하나를 담고, "
                "한 문장에 여러 일정/할 일/알림 의도가 섞이면 여러 StructuredRequest로 나눈다."
            ),
            (
                "개인 일정 생성 요청에서 personal_create_schedule 도구를 호출했다면, "
                "그 결과 JSON의 created_schedule을 읽어 title/date/start_time/end_time/members 필드를 채운다. "
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        (
            "너는 사용자의 한국어 자연어 요청을 구조화하는 Week 2 요청 구조화 비서다. "
            f"오늘 날짜는 {current_app_date_iso()}이며, 상대 날짜는 이 날짜를 기준으로 "
            "YYYY-MM-DD 형식으로 계산한다."
        ),
        (
            "자연어 요청을 StructuredRequest 필드로 구조화한다. "
            "kind는 personal_schedule/group_schedule/todo/reminder/unknown 중에서 고르고, "
            "title/date/start_time/end_time/members/priority/reason/original_text를 채운다. "
            "확실하지 않은 날짜/시간은 None, 언급 없는 멤버는 빈 list로 둔다."
        ),
        (
            "이미 Week 1 tool이 만든 JSON(created_schedule 등)을 받은 경우에는 도구를 다시 호출하지 말고, "
            "그 payload를 읽어 StructuredRequest 필드로 옮겨 구조화 결과만 만든다."
        ),
        (
            "Week 2에서는 SQLite 저장, RAG 색인, 외부 멤버 일정 조율을 하지 않는다. "
            "요청을 구조화하는 것까지가 Week 2의 역할이다."
        ),
    ]
...
def build_week02_agent() -> object:
    ...
    #       (참고: 이 프록시 모델은 native structured output 모드에서 JSON 뒤에 여분 텍스트를 붙여
    #        StructuredOutputValidationError를 일으키므로, ToolStrategy(...)로 감싸 function-calling 방식으로 강제한다.)
