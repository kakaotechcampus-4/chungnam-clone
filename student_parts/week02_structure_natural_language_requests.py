...
class StructuredRequest(BaseModel):
    ...
    title: str | None = Field(
        default=None, description="일정/할 일의 제목입니다. 확실하지 않으면 None입니다."
    )
    date: str | None = Field(
        default=None, description="YYYY-MM-DD 형식의 날짜입니다. 확실할 때만 채웁니다."
    )
    start_time: str | None = Field(
        default=None, description="HH:MM 형식의 시작 시각입니다. 확실할 때만 채웁니다."
    )
    end_time: str | None = Field(
        default=None, description="HH:MM 형식의 종료 시각입니다. 확실할 때만 채웁니다."
    )
    members: list[str] = Field(
        default_factory=list, description="참석자/관련 멤버 목록입니다. 모르면 빈 list입니다."
    )
    priority: str | None = Field(
        default=None, description="할 일의 우선순위입니다. 확실하지 않으면 None입니다."
    )
    reason: str | None = Field(
        default=None, description="이 요청을 이렇게 구조화한 판단 근거입니다."
    )
    original_text: str = Field(
        default="", description="구조화 이전의 원문 텍스트입니다."
    )
...
class StructuredRequestBatch(BaseModel):
    ...
    base_date: str = Field(
        ...
        description="상대 날짜(예: 내일, 다음 주 화요일)를 해석할 때 기준이 되는 오늘 날짜(YYYY-MM-DD)입니다.",
    )
        ...
        [
            *week02_prompt_parts(),
            (
                ...
                "요청이 하나뿐이어도 requests 목록 안에 StructuredRequest 하나를 담아."
            ),
            (
                "개인 일정 생성 요청이라면 personal_create_schedule tool 결과 JSON의 created_schedule을 "
                "읽어서 title/date/start_time/end_time/members 같은 필드를 채워."
            ),
        ]
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        (
            "이제부터 너는 자연어 요청과 Week 1 tool 결과를 구조화된 데이터로 정리하는 "
            "Week 2 요청 구조화 agent야. "
            f"오늘 날짜는 {current_app_date_iso()}이야. '내일', '모레', '다음 주 화요일' 같은 "
            "상대적인 날짜 표현은 이 날짜를 기준으로 계산해."
        ),
        (
        ...
        (
            "개인 일정 생성 요청이라면 personal_create_schedule tool을 호출해 created_schedule JSON을 "
            "받아온 뒤, 그 JSON을 다시 tool 호출 없이 그대로 읽어서 StructuredRequestBatch 필드를 채워."
        ),
        (
            "Week 2에서는 SQLite 저장, RAG 검색, 외부 멤버 일정 조율을 하지 않아. "
            "구조화 결과를 만들어 반환하는 것까지만 담당해."
        ),
    ]
...
def build_week02_agent() -> object:
    ...
    if _WEEK02_AGENT is None:
        _WEEK02_AGENT = create_agent(
            model=chat_model(),
            tools=week02_tools(),
            # response_format=StructuredRequestBatch(ProviderStrategy)는 이 프록시 모델에서 native
            # structured output이 JSON 뒤에 extra data를 붙여 StructuredOutputValidationError를 냈다.
            # ToolStrategy(StructuredRequestBatch)로 바꾸면 function-calling 경유로 스키마를 강제해
            # 같은 프롬프트에서 정상 파싱됨을 확인했다.
