...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(
        description=(
            ...
            "todo(할 일), reminder(리마인더), unknown(판단 불가) 중 하나."
        )
    )
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        ...
        description="구조화된 요청 목록. 요청이 하나뿐이어도 StructuredRequest 하나를 담은 목록으로 만든다.",
    )
    base_date: str = Field(
        default_factory=current_app_date_iso,
        description="상대 날짜 해석의 기준일. YYYY-MM-DD 형식의 오늘 날짜다.",
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        Week 1 tool 결과 JSON을 받은 경우 다시 tool을 호출하지 않고 payload를 읽어 structured_response로 만든다.
