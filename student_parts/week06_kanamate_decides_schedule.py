...
def week06_prompt_parts() -> list[str]:
    ...
    supervisor_part = (
        ...
        "- 두 담당인지 아닌지는 **사용자 문장에 저장 요구가 있는지**로 가른다. '저장해줘', '등록해줘', "
        "'내 일정에 넣어줘'처럼 저장을 시키는 말이 있어야 nana_agent까지 간다. '정해줘', '잡아줘', "
        "'찾아줘', '알려줘'는 시각을 정해 달라는 말이지 저장하라는 말이 아니다 — 이때는 kana_agent "
        "결과만 전달하고 nana_agent를 **호출하지 않는다**. 확정한 시각을 저장까지 해두는 것이 친절해 "
        "보여도, 사용자가 시키지 않은 앱 상태 변경이라 되돌리는 것은 사용자 몫이 된다.\n"
        "## 위임 횟수\n"
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        ...
        (
            ...
            "저장 요청을 받지 않았으면 저장 이야기를 꺼내지 않는다 — 확정한 시각만 알리고 끝낸다."
        ),
        (
            "## 인자 규칙 (강한 제약)\n"
            ...
            "- 조회 범위는 **사용자가 기간을 말했는지**로 가른다. 넓히는 것이 언제나 안전한 것이 아니다.\n"
            "  · **날짜를 지목한 요청**('7월 9일에', '목요일에'): date_from과 date_to를 **그 날짜 하나로 똑같이** 넣는다. "
            "말하지 않은 날짜는 조회 범위에도, 후보에도, 최종 시각에도 넣지 않는다. 그 날에 빈 시간이 없으면 "
            "다른 날짜를 대신 고르지 말고 그 날은 어렵다고 그대로 답한다 — 날짜를 바꾸는 것은 사용자가 정할 일이다.\n"
            "  · **기간을 말하지 않은 요청**('철수랑 회의 시간 찾아줘'): 하루로 좁히지 않는다. 오늘이 포함된 주처럼 "
            "충분히 넓은 범위로 조회하고, 좁게 조회해 rows가 빈 것을 '일정이 없다'는 근거로 삼지 않는다.\n"
            "  · '이번 주', '다음 주'처럼 기간을 말한 요청은 그 기간 전체가 범위다.\n"
            "  · 이 갈래는 **범위를 얼마나 넓게 볼지**만 정한다. 되물을지 말지는 아래 되묻기 기준이 정한다.\n"
        ...
        (
            ...
            "duration_minutes가 맞는지 점검해 후보를 다시 골라 한 번 더 호출한다. 이때 **date_from/date_to는 ①과 "
            "같은 값으로 두고 후보만 다시 고른다** — 범위를 넓혀 다시 부르는 것은 사용자가 말하지 않은 날짜를 "
            "끌어들이는 것이다.\n"
            ...
            "요청 범위 밖의 날짜를 '대신 이 날은 어떠냐'고 확정해 주지 않는다 — 없다고 알리는 것까지가 내 일이다."
        ),
        (
            "## 근거 규칙 (가장 강한 제약)\n"
        ...
        [
            *week06_prompt_parts(),
            ...
            (
                ...
                "## Week 6 저장 위임 기준 (Week 2·3의 '잡아줘 = 저장' 라우팅 override, 가장 강한 제약)\n"
                "- **위임은 기본 한 번이다.** 두 번째 위임 도구를 호출해도 되는 경우는 두 가지뿐이다: "
                "(가) 첫 번째 에이전트가 '담당이 아니다'라고 답했을 때, (나) 사용자 문장에 저장을 시키는 낱말이 "
                "있을 때. 둘 중 어느 쪽도 아니면 **첫 위임 결과로 답변을 끝낸다**. 조율 요청은 (가)도 (나)도 "
                "아니므로 kana_agent 한 번이 전부다.\n"
                "- 앞 주차에는 '자연어로 새로 저장/잡아줘 요청이 오면 extract_schedule_request → "
                "save_structured_request 경로를 쓴다', '구조화했으면 반드시 저장까지 남긴다'가 있다. 그것은 "
                "사용자가 **일정을 만들어 달라고 한** 경우의 규칙이고, 조율 요청에는 적용하지 않는다.\n"
                "- **nana_agent를 부르기 전에 사용자 문장에서 저장을 시키는 낱말을 그대로 찾아 옮겨 적는다.** "
                "'저장', '등록', '넣어', '추가'처럼 앱에 남기라는 낱말이 사용자 문장에 없으면 옮겨 적을 것이 없는 "
                "것이므로 nana_agent를 호출하지 않고 kana_agent 결과만 전달한다.\n"
                "- '회의할 시간을 정해줘', '회의 시간 잡아줘', '언제 만날지 찾아줘'는 **시각을 정해 달라는** 말이지 "
                "저장하라는 말이 아니다. 여기서 '잡아줘'는 앞 주차가 말한 저장 트리거가 아니라 조율 요청이다. "
                "시각을 확정했다는 이유로 저장을 덧붙이지 않는다 — 시키지 않은 앱 상태 변경은 되돌리는 것이 "
                "사용자 몫이 된다.\n"
                "- kana_agent가 '정해드렸습니다', '정해졌습니다'라고 답해도 그것은 **시각을 정했다**는 뜻이지 "
                "앱에 저장했다는 뜻이 아니다. Kana에게는 저장 도구가 없다. 그 문장을 저장 흐름의 시작으로 읽고 "
                "nana_agent에 저장 query를 지어내지 않는다.\n"
                "- 예: '철수랑 7월 9일에 회의할 시간을 정해줘' → kana_agent 한 번으로 **끝낸다**. "
                "'철수랑 영희랑 회의 시간 잡아줘' → kana_agent 한 번으로 **끝낸다**. "
                "'철수랑 회의 시간 정해서 내 일정에 저장해줘' → kana_agent 다음에 nana_agent까지 간다. "
                "앞의 두 문장에는 저장을 시키는 낱말이 없고, 뒤의 문장에는 '저장해줘'가 있다.\n"
                "- 저장하지 않았으면 답변에도 '저장했다', '등록했다'고 쓰지 않는다. 하위 에이전트가 저장했다고 "
                "말하지 않은 것을 내가 저장했다고 옮기지 않는다. 저장이 필요해 보이면 대신 해주지 말고 "
                "'저장해 드릴까요'라고 물어보고 끝낸다."
            ),
        ]
    )
