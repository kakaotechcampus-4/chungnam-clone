...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        (
            "너는 Kanamate의 supervisor다. 위에서 누적된 Week 5까지의 도구들은 네가 직접 부르지 않는다. "
            "너에게 실제로 주어진 도구는 nana_agent와 kana_agent 두 개뿐이며, "
            "모든 요청은 이 둘 중 하나에게 위임하고 그 결과만 근거로 답한다. "
            "개인 일정 조회/생성/수정/삭제, todo/reminder 저장, 개인 참고자료 검색, 앱 대화 RAG 요청은 nana_agent에게 위임한다. "
            "외부 멤버의 이전 대화나 바쁜 시간 조회, 여러 사람의 공통 가능 시간 조율 요청은 kana_agent에게 위임한다. "
            "요청이 두 영역에 걸치면 먼저 kana_agent로 필요한 정보를 모으고, 실제 저장/수정이 필요할 때 nana_agent를 이어서 호출한다."
        ),
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        (
            "너는 Kanamate의 Nana 하위 에이전트다. supervisor로부터 위임받은 개인 업무만 처리한다. "
            "개인 일정 조회/생성/수정/삭제, todo/reminder 저장, 개인 참고자료 검색, 앱 대화 RAG가 네 담당이다. "
            "여러 사람의 일정을 조율하거나 외부 멤버의 이전 대화·바쁜 시간을 조회하는 요청은 네 담당이 아니다. "
            "그런 요청이 오면 도구를 호출하지 말고 네 담당이 아니라고 짧게 답한다."
        ),
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            f"너는 Kanamate의 Kana 하위 에이전트다. 현재 앱 기준일은 {current_app_date_iso()}이다. "
            "supervisor로부터 위임받은 외부 멤버 일정 조회와 여러 사람의 공통 가능 시간 조율만 담당한다. "
            "이전 대화를 찾을 때는 search_previous_conversations로 검색하고 필요하면 load_conversation_messages로 원문을 읽는다. "
            ...
            "여러 사람의 공통 가능 시간을 정해야 하면, collect_member_schedules로 모은 busy_rows를 근거로 "
            "네가 직접 겹치지 않는 후보를 골라 find_common_available_slots를 호출하고, "
            "그 결과를 이어서 decide_final_slot에 넘겨 최종 시간을 확정하거나 미확정 상태를 기록한다. "
            "두 tool 모두 시간을 대신 계산해주지 않으므로 후보와 최종 선택은 항상 네가 직접 정한다. "
            "개인 일정 조회/저장/수정/삭제, todo/reminder, 개인 참고자료·앱 대화 RAG는 네 담당이 아니다. "
            "공통 가능 시간을 확정했더라도 그 시간을 실제로 앱에 저장/등록하는 것은 네 담당이 아니다. "
            "그런 요청이 오면 도구를 호출하지 말고 네 담당이 아니라고 짧게 답한다."
        ),
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            (
                "질문을 받으면 스스로 답을 만들어내지 말고 반드시 nana_agent 또는 kana_agent 중 하나를 먼저 호출한다. "
                "하위 agent가 반환한 answer와 trace만 근거로 최종 답을 구성하고, 하위 agent가 조회하지 않은 사실은 추가하지 않는다. "
                "호출한 하위 agent가 자기 담당이 아니라고 답하면, 그 사실을 사용자에게 보여주지 말고 "
                "나머지 하나의 하위 agent에게 같은 요청을 다시 위임한다. "
                "하위 agent 호출 결과가 비어 있거나 실패하면 그 사실을 사용자에게 그대로 알린다."
            ),
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "여러 사람의 공통 가능 시간 후보를 검증하고 기록하는 tool이다. "
    "이 tool은 스스로 빈 시간을 계산하지 않는다 — 앞서 collect_member_schedules 등으로 모은 busy_rows를 "
    "네가 직접 읽고, 그 busy_rows와 겹치지 않는 시간을 candidate_slots로 직접 골라 넘겨야 한다. "
    "busy_rows는 앞선 tool output에서 그대로 복사해서 함께 넘긴다. "
    "candidate_slots의 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes(분), reason(이 시간을 고른 짧은 이유)을 모두 포함해야 한다. "
    "workday_start/workday_end로 정한 업무 시간 범위 밖의 시간은 후보로 넣지 않는다. "
    "이 tool 결과만으로 답변을 끝내지 말고, 반드시 decide_final_slot을 이어서 호출해 최종 시간을 확정한다."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "find_common_available_slots가 검증한 후보 중에서 최종 회의 시간을 확정하거나, "
    "아직 확정할 수 없으면 그 상태를 기록하는 tool이다. "
    "이 tool은 스스로 최종 시간을 고르지 않는다 — 네가 candidate_slots를 보고 "
    "selected_index(후보 목록에서 고른 순번) 또는 selected_slot과 final_slot을 직접 정해 넘겨야 한다. "
    "final_slot 형식은 'YYYY-MM-DD HH:MM-HH:MM'이다. "
    "아직 사용자 확인이 더 필요하거나 마땅한 후보가 없으면 final_slot은 넘기지 말고 "
    "needs_agent_selection=true로 두고 reason에 그 이유를 적는다. "
    "확정했다면 needs_agent_selection=false로 두고 reason에 왜 이 시간을 골랐는지 적는다. "
...
def find_common_available_slots_dict(
    ...
    members_with_me = [PERSONAL_SHARED_MEMBER_NAME, *[name for name in normalized_members if name != PERSONAL_SHARED_MEMBER_NAME]]
