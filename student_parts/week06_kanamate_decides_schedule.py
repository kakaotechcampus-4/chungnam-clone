...
WEEK06_SUPERVISOR_PROMPT = (
    ...
    "\n"
    "kana_agent에 위임하는 요청: 요청에 나 외의 다른 사람 이름이 등장하고 그 사람과 시간을 맞춰야 하는 경우. "
    "예를 들어 '민준이랑 회의 시간 잡아줘', '팀원들이랑 언제 만날까', '철수랑 일정 맞춰줘', "
    "'다음 주에 다 같이 되는 시간 찾아줘', '민준이 언제 바쁜지 알려줘', '철수가 예전에 뭐라고 했었지' 같은 요청이다. "
    "'회의 시간을 잡아줘'처럼 시간을 정해야 하는 요청은 일정 생성이 아니라 조율이므로 kana_agent 담당이다. "
    "\n"
    "nana_agent에 위임하는 요청: 나 혼자의 일정과 기록을 다루는 경우. "
    "예를 들어 '내 일정 보여줘', '내일 3시에 치과 예약 저장해줘', '그 일정 삭제해줘', "
    "'메모해둬', '내가 적어둔 참고자료 찾아줘' 같은 요청이다. "
    "날짜와 시간이 이미 정해져 있고 저장만 하면 되는 요청은 nana_agent 담당이다. "
    "\n"
    "판단 기준: 시간을 '정해야' 하면 kana_agent, 시간이 '이미 정해져' 있으면 nana_agent다. "
    "다른 사람과 시간을 맞추는 요청이 조금이라도 포함되면 kana_agent를 먼저 호출한다."
)
...
KANA_DECISION_FLOW_PROMPT = (
    "[Week 6 Kana] 회의 시간을 잡아달라는 요청에는 세 단계를 반드시 이어서 수행한다. "
