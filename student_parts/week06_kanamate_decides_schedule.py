...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        "너는 KanaMate 의 supervisor 다. 직접 도구로 일하지 않고, 요청을 nana_agent 또는 "
        "kana_agent 에게 위임한 뒤 그 결과만 근거로 답한다.",
        "내 개인 일정을 만들거나 조회하거나 수정하거나 삭제하는 일, 할 일과 알림 저장, "
        "내가 적어 둔 참고자료나 내가 나눈 지난 대화를 찾는 일은 nana_agent 에게 위임한다.",
        "다른 사람(외부 멤버)의 지난 대화나 일정을 찾는 일, 여러 사람의 바쁜 시간을 모아 "
        "공통으로 비는 회의 시간을 정하는 일은 kana_agent 에게 위임한다.",
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        "너는 Nana. 사용자 본인의 개인 일정과 기억을 담당하는 하위 에이전트다. "
        "개인 일정 생성, 조회, 수정, 삭제와 할 일, 알림 저장, 내가 적어 둔 참고자료와 "
        "내가 나눈 지난 대화 검색을 네 도구로 처리한다.",
        "다른 사람의 일정을 모아 회의 시간을 맞추는 그룹 조율은 네 담당이 아니다. "
        "그런 요청이 오면 그건 Kana 담당이라고 한 문장으로 알린다.",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        f"오늘 날짜는 {current_app_date_iso()} 이다. 상대 날짜 표현은 이 날짜를 기준으로 환산한다.",
        "너는 Kana. 다른 사람들의 일정을 모아 회의 시간을 맞추는 하위 에이전트다. "
        "외부 멤버의 지난 대화 검색, 그 대화에서 일정 추출, 공유 일정 조회를 네 도구로 처리한다.",
        "여러 사람과 회의 시간을 잡아달라는 요청은 이 순서로 처리한다. "
        "먼저 collect_member_schedules 로 내 일정과 외부 멤버의 바쁜 시간을 한 번에 모은다. "
        "그 결과의 rows 를 busy_rows 로 삼아, 겹치지 않는 후보를 직접 골라 "
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "요청을 받으면 먼저 nana_agent 와 kana_agent 중 담당을 정해 반드시 하나를 호출한다. "
            "직접 답을 만들지 말고, 위임한 하위 에이전트가 돌려준 결과만 근거로 사용자에게 답한다.",
            "그룹 회의 시간 조율처럼 kana_agent 가 최종 시간을 정한 경우, 그 시간과 이유를 "
            "사용자에게 그대로 정리해서 전한다.",
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "여러 사람이 공통으로 비는 회의 시간 후보를 검증한다. "
    "이 도구는 후보를 대신 계산하지 않는다. 네가 busy_rows 를 직접 읽고, 어떤 busy row 와도 "
    "겹치지 않는 시간을 candidate_slots 로 직접 골라 넘겨야 한다. "
    "candidate_slots 의 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes, reason 을 포함한다. "
    "busy_rows 는 앞서 collect_member_schedules 가 돌려준 rows 를 그대로 복사해 넘긴다. "
    "이 검증으로 답을 끝내지 말고, 검증된 후보를 가지고 decide_final_slot 을 이어서 호출한다."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "회의 시간 후보 중 최종 시간을 기록한다. "
    "이 도구는 최종 시간을 대신 고르지 않는다. 네가 selected_index(또는 selected_slot)로 후보를 "
    "직접 고르고, final_slot 을 'YYYY-MM-DD HH:MM-HH:MM' 형식으로 직접 채워 넘긴다. "
    "아직 고를 수 없으면 final_slot 은 null, needs_agent_selection 은 true 로 둔다. "
    "근거를 남기도록 candidate_slots, busy_rows, member_names, date_from, date_to, reason 도 함께 넘긴다."
)
...
def kana_agent(query: str) -> str:
    ...
    # 하위 trace 의 tool 결과에서 최종 시간 결정을 끌어올려 supervisor 가 바로 쓸 수 있게 한다.
    final_slot_payload: dict[str, Any] | None = None
    final_decision_payload: dict[str, Any] | None = None
    for event in events:
