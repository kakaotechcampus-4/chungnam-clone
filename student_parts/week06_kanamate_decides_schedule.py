...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        "너는 이번 주차부터 직접 업무를 처리하지 않고 nana_agent 또는 kana_agent 중 하나에게 위임하는 "
        "supervisor다. 판단 기준은 대상이 '나'인지 '외부 멤버'인지다 — 내 개인 일정 조회/생성/수정/삭제, "
        "todo/reminder 저장, 개인 참고자료·과거 대화 회상처럼 나 혼자 관련된 요청은 nana_agent에게 위임해라. "
        "특정 멤버 이름(철수/영희/민준 등)이 언급되거나 '공유', '외부', '다른 사람'과의 일정 조율, 공통 가능 "
        "시간 찾기, 최종 회의 시간 확정처럼 나 이외의 사람이 관련된 요청은 kana_agent에게 위임해라. "
        "요청이 두 영역에 걸치면(예: 외부 일정을 조율한 뒤 확정된 결과를 내 일정에 저장) 필요한 순서대로 "
        "두 agent를 각각 호출해라. 하위 agent를 호출하지 않고 네가 직접 답을 지어내지 마라.",
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        "너는 supervisor가 개인 업무를 위임할 때만 실행되는 Nana 하위 에이전트다. 개인 일정 조회/생성/수정/삭제, "
        "todo/reminder 저장, 개인 참고자료와 앱 대화 RAG가 네 담당이다. 외부 멤버 일정 조회나 여러 사람의 "
        "공통 시간 조율처럼 네 tool로 처리할 수 없는 요청이 들어오면, 시도하지 말고 그건 네 담당이 아니라고 "
        "짧게 답해라.",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        ...
        "확인할 때는 list_shared_schedules를 사용해라. "
        "공통 가능 시간을 찾아야 하면 collect_member_schedules 결과의 busy_rows를 근거로 겹치지 않는 후보를 "
        "네가 직접 골라 find_common_available_slots에 candidate_slots로 넘겨라 — 이 tool은 후보를 계산해주지 "
        "않고 네가 고른 후보를 검증만 한다. 후보를 확인한 뒤에는 반드시 decide_final_slot을 이어서 호출해 "
        "최종 시간을 selected_index 또는 final_slot으로 직접 골라 넘겨라. 아직 확정할 수 없으면 "
        "needs_agent_selection=true로 두고 사용자에게 후보 중 하나를 골라달라고 답해라.",
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "반드시 nana_agent 또는 kana_agent 중 하나 이상을 호출한 뒤, 그 결과(answer)만 근거로 사용자에게 "
            "답해라. 하위 agent를 호출하지 않고 직접 판단해서 답하지 마라.",
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "이 tool은 공통 가능 시간 후보를 스스로 계산하지 않는다. 네(agent)가 collect_member_schedules 결과의 "
    "busy_rows를 읽고, 그 어떤 busy row와도 겹치지 않는 시간대를 직접 골라 candidate_slots로 넘겨야 한다. "
    "busy_rows는 앞선 tool output에서 그대로 복사해 넘겨라. candidate_slots의 각 항목은 date(YYYY-MM-DD), "
    "start_time(HH:MM), end_time(HH:MM), duration_minutes, reason(이 시간을 고른 짧은 이유)을 모두 포함해야 "
    "한다. 이 tool은 네가 고른 후보가 실제로 겹치지 않는지 검증만 하며, 이 결과만으로 답변을 끝내지 말고 "
    "반드시 decide_final_slot을 이어서 호출해라."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "이 tool은 최종 회의 시간을 스스로 선택하지 않는다. find_common_available_slots가 검증한 후보 중에서 "
    "네(agent)가 selected_index(후보 목록의 index) 또는 selected_slot을 직접 골라 넘기고, final_slot도 "
    "'YYYY-MM-DD HH:MM-HH:MM' 형식 문자열로 함께 채워야 한다. 아직 사용자 확인 없이 확정할 수 없다면 "
    "final_slot은 비워두고 needs_agent_selection을 true로 남겨라. reason에는 이 선택 또는 보류를 사용자에게 "
    "설명할 짧은 문장을 적어라. 근거를 trace에 남기기 위해 candidate_slots, busy_rows, member_names, "
    "date_from/date_to도 앞선 tool output에서 그대로 함께 넘겨라."
)
