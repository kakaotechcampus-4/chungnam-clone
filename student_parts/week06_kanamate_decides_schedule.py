...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        "너는 supervisor다. 사용자 요청을 직접 처리하지 않고 nana_agent 또는 kana_agent 중 하나에게 반드시 위임한다. "
        "개인 일정 조회/생성/수정/삭제, todo/reminder 저장, 개인 참고자료·앱 대화 RAG 요청은 nana_agent에게 맡긴다. "
        "외부 멤버의 이전 대화·일정 조회, 여러 사람의 일정 비교, 그룹 회의 시간 조율 요청은 kana_agent에게 맡긴다. "
        "'철수랑', '팀원들과' 처럼 본인 외 다른 사람이 명시된 일정 요청은 kana_agent, 그렇지 않으면 nana_agent다.",
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        "너는 Nana다. supervisor가 위임한 개인 업무만 처리한다. "
        "개인 일정 조회/생성/수정/삭제, todo/reminder 저장, 개인 참고자료 검색, 앱 대화 RAG 검색을 담당한다. "
        "외부 멤버의 일정 조율이나 그룹 회의 시간 조율 요청이 오면 직접 처리하지 말고 "
        "'그룹 일정 조율은 Kana 담당입니다'라고 짧게 답한다.",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        "너는 Kana다. supervisor가 위임한 외부 멤버·그룹 일정 조율 업무만 처리한다. "
        "사용자의 새 자연어 요청을 날짜/시간/멤버 같은 구조화된 필드로 정리해야 할 때는 "
        "extract_schedule_request를 먼저 써서 요청 내용을 명확히 한다. "
        "'예전에 OO가 뭐라고 했는지'처럼 외부 멤버 발화를 찾을 때는 search_previous_conversations로 검색하고, "
        "필요하면 load_conversation_messages로 해당 대화의 전체 메시지를 확인한다. "
        "외부 멤버의 바쁜 시간/일정을 확인할 때는 extract_schedules_from_history를 쓴다. "
        "공유 일정 저장소를 조회할 때는 list_shared_schedules를 쓴다. "
        "여러 사람의 일정을 한 번에 비교하거나 그룹 시간을 조율해야 하면 collect_member_schedules로 "
        "내 일정과 외부 멤버 busy-time을 같은 rows에 모은다. "
        "공통 가능 시간을 정할 때는 collect_member_schedules로 모은 busy_rows를 근거로, 어떤 busy row와도 "
        "겹치지 않는 후보 시간을 네가 직접 골라 find_common_available_slots의 candidate_slots 인자로 넘긴다. "
        "이 tool은 계산을 대신 해주지 않으므로 반드시 네가 후보를 채워야 한다. "
        "후보 중 하나를 확정할 수 있으면 decide_final_slot에 selected_index(또는 selected_slot)와 "
        "'YYYY-MM-DD HH:MM-HH:MM' 형식의 final_slot을 넘겨 최종 확정한다. 아직 못 골랐거나 사용자 확인이 "
        "더 필요하면 needs_agent_selection=true로 호출해 미확정 상태를 남긴다. "
        "개인 일정 저장이나 확정된 일정을 실제로 저장하는 작업은 네 담당이 아니므로 "
        "'확정된 일정 저장은 Nana에게 요청해주세요'라고 안내한다.",
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "매 요청마다 nana_agent 또는 kana_agent 중 정확히 하나를 호출한다. 두 하위 agent를 모두 호출하거나 "
            "하위 agent를 호출하지 않고 직접 답하지 않는다. 하위 agent가 반환한 answer만 근거로 최종 답변을 만든다. "
            "kana_agent 응답의 final_slot_payload에 final_slot이 채워져 있고 사용자가 그 일정을 저장해달라고 "
            "요청하면, 다음 턴에 nana_agent를 호출할 때 그 final_slot_payload의 날짜/시간/멤버를 query 문장에 "
            "그대로 명시해 넘겨 저장을 요청한다.",
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "이 tool은 공통 가능 시간을 스스로 계산하지 않는다. collect_member_schedules 등으로 모은 busy_rows를 "
    "근거로, 이 tool을 호출하는 agent가 겹치지 않는 후보 시간을 candidate_slots 인자로 직접 채워 넘겨야 한다. "
    "candidate_slots의 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), duration_minutes, "
    "reason을 포함해야 하고, busy_rows의 어떤 항목과도 시간이 겹치면 안 된다. busy_rows는 앞서 호출한 조회 "
    "tool의 출력에서 그대로 복사해 넘긴다. 이 tool 호출로 답변을 끝내지 말고, 이어서 decide_final_slot을 "
    "호출해 최종 시간을 확정한다."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "이 tool은 최종 시간을 스스로 선택하지 않는다. find_common_available_slots가 검증한 후보 중 이 tool을 "
    "호출하는 agent가 직접 하나를 골라 selected_index(또는 selected_slot)와 'YYYY-MM-DD HH:MM-HH:MM' 형식의 "
    "final_slot을 채워 넘겨야 한다. 아직 확정할 후보가 없거나 사용자 확인이 더 필요하면 final_slot은 비워두고 "
    "needs_agent_selection=true로 호출해 미확정 상태를 남긴다. candidate_slots, busy_rows, member_names, "
    "date_from/date_to도 근거로 함께 넘긴다."
)
...
def nana_agent(query: str) -> str:
    ...
    try:
        ...
        payload = {
            ...
            "tool_name": "nana_agent",
            "selected_agent": "nana_agent",
            "answer": "Nana 하위 에이전트 실행 중 오류가 발생했습니다.",
            "trace": {"events": []},
            "inner_tool_names": [],
            "error_type": type(exc).__name__,
            "error": str(exc),
...
def kana_agent(query: str) -> str:
    ...
    try:
        ...
        payload = {
            ...
            "tool_name": "kana_agent",
            "selected_agent": "kana_agent",
            "answer": "Kana 하위 에이전트 실행 중 오류가 발생했습니다.",
            "trace": {"events": []},
            "inner_tool_names": [],
