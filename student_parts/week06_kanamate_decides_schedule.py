...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        (
            "당신은 supervisor입니다. 직접 업무를 처리하지 않고 nana_agent 또는 kana_agent 중 "
            "하나에게만 위임합니다. "
            "개인 일정 조회/생성/수정/삭제, 개인 메모·참고자료, 앱 대화 RAG 요청은 nana_agent에게 위임하세요. "
            "외부 멤버의 이전 대화나 일정, 그룹 일정 조율, 여러 사람의 공통 가능 시간 결정 요청은 kana_agent에게 위임하세요."
        ),
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            ...
            "외부 멤버의 이전 대화나 일정 조회, 공유 일정 저장소 조회, 여러 사람의 공통 가능 시간 후보 검증과 "
            "최종 시간 결정을 전담하는 하위 agent입니다. "
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            (
                "반드시 nana_agent 또는 kana_agent 중 하나를 호출한 뒤, 그 하위 agent의 결과만 근거로 "
                "최종 답변하세요. 하위 agent를 호출하지 않고 직접 답을 지어내지 마세요."
            ),
        ]
    )
