...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        ...
        "너는 supervisor다. 직접 일정을 조회/생성/저장하지 않고 반드시 nana_agent 또는 kana_agent 도구로 위임한다. "
        "개인 일정 생성/조회/수정/삭제, 할 일·리마인더 저장, 개인 참고자료·앱 대화 RAG 관련 요청은 nana_agent에게 위임한다. "
        "외부 멤버 일정 조회, 공유 일정 저장소 확인, 여러 사람의 공통 가능 시간 찾기와 최종 회의 시간 확정은 kana_agent에게 위임한다.",
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        "너는 나나다. 개인 일정 생성/조회/수정/삭제, 할 일·리마인더 저장, 개인 참고자료·앱 대화 기록 검색만 담당한다. "
        "그룹 일정 조율이나 외부 멤버 일정 관련 요청이 오면 네 담당이 아니라고 짧게 답한다.",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        ...
        f"너는 카나다. 오늘은 {current_app_date_iso()}이다. "
        "외부 멤버 일정 조회, 공유 일정 저장소 확인, 여러 사람의 공통 가능 시간 후보 검증, 최종 회의 시간 확정을 담당한다. "
        "확정된 개인 일정 저장은 네 담당이 아니므로 필요하면 나나에게 요청하라고만 안내한다. "
        "그룹 일정 요청이 오면 collect_member_schedules 등으로 busy time을 모으고, "
        "네가 직접 겹치지 않는 후보를 골라 find_common_available_slots에 candidate_slots로 넘겨 검증한 뒤, "
        "최종 시간을 골라 decide_final_slot에 final_slot으로 넘겨 확정한다.",
    ]
    ...
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            ...
            "실행할 때 반드시 nana_agent 또는 kana_agent 중 하나를 호출하고, 그 결과(answer)만 근거로 최종 답을 작성한다. "
            "두 하위 agent가 할 일을 네가 대신 처리하지 않는다.",
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    ...
    "이 tool은 공통 가능 시간을 자동으로 계산하지 않는다. busy_rows(이전 tool 결과에서 그대로 복사)를 "
    "직접 읽고, 겹치지 않는 후보를 candidate_slots 인자로 네가 채워서 호출해야 한다. "
    "candidate_slots 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes, reason을 포함해야 하며 어떤 busy_rows와도 겹치면 안 된다. "
    "이 tool 결과만으로 답변을 끝내지 말고, 이어서 decide_final_slot을 호출해 최종 시간을 확정해야 한다."
)
...
DECIDE_FINAL_SLOT_DESCRIPTION = (
    ...
    "이 tool은 최종 시간을 자동으로 고르지 않는다. candidate_slots 중 네가 직접 고른 항목을 "
    "selected_index(또는 selected_slot)와 final_slot('YYYY-MM-DD HH:MM-HH:MM' 형식)으로 채워서 호출해야 한다. "
    "아직 고르지 못했다면 final_slot은 비워두고 needs_agent_selection=true로 호출한다. "
...
def find_common_available_slots(
    ...
    result = find_common_available_slots_dict(
        ...
        limit=limit, busy_rows=busy_rows, candidate_slots=candidate_slots, llm_reason=llm_reason,
    )
...
def decide_final_slot(
    ...
    payload = decide_final_slot_payload(
        ...
        member_names=member_names, date_from=date_from, date_to=date_to,
        duration_minutes=duration_minutes, final_slot=final_slot,
        needs_agent_selection=needs_agent_selection, reason=reason, busy_rows=busy_rows,
    )
