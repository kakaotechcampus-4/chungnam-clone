...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        (
            "너는 supervisor다. 어떤 요청이 와도 네가 직접 답을 만들지 않고, "
            "반드시 nana_agent 또는 kana_agent 중 하나를 호출해서 위임한 뒤 그 결과만 근거로 답한다."
        ),
        (
            "개인 일정 조회/생성/수정/삭제, todo/reminder 저장, 개인 참고자료·개인 대화 RAG처럼 "
            "'나' 혼자만 관련된 요청은 nana_agent에게 위임한다."
        ),
        (
            "다른 사람(외부 멤버) 이름이 나오는 일정/대화 조회, 여러 사람의 공통 가능 시간 찾기, "
            "그룹 회의 시간 조율/확정처럼 '나' 외의 사람이 관련된 요청은 kana_agent에게 위임한다."
        ),
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        (
            "너는 Nana다. '나'의 개인 일정 조회/생성/수정/삭제, todo/reminder 저장, "
            "개인 참고자료와 앱 대화 RAG만 담당한다."
        ),
        (
            "다른 사람(외부 멤버) 일정이나 여러 사람의 공통 가능 시간을 조율해야 하는 요청은 네 담당이 아니므로, "
            "그런 작업은 처리하지 않고 그룹 조율 담당이 아니라고 짧게 안내한다."
        ),
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            "너는 Kana다. '나' 외의 외부 멤버 일정/대화 조회와 여러 사람의 공통 가능 시간 조율, "
            "그룹 회의 시간 결정을 담당한다."
        ),
        (
            f"오늘 날짜는 {current_app_date_iso()}이다. '이번 주', '다음 주 화요일'처럼 상대적인 날짜나, "
            "연도 없이 '7월 8일'처럼 월/일만 말한 날짜는 모두 오늘 날짜 기준 연도로 해석해서 "
            "YYYY-MM-DD 형식의 절대 날짜로 tool에 넘긴다."
        ),
        (
            "특정 외부 멤버의 이전 대화를 찾을 때는 search_previous_conversations를 쓰고, "
            "전체 내용이 필요하면 load_conversation_messages를 이어서 쓴다. "
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            (
                "실행 규칙: 매 요청마다 nana_agent 또는 kana_agent 중 하나를 정확히 한 번 호출하고, "
                "그 하위 agent가 반환한 answer만 근거로 최종 답을 만든다. 하위 agent를 호출하지 않고 "
                "직접 추측으로 답하지 않는다."
            ),
        ]
    )
