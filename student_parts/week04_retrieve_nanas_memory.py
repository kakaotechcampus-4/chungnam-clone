    ...
    return [
        *week03_prompt_parts(),
        (
            ...
            "⑥ 종류를 특정하지 않은 일정 조회(예: '내일 일정 뭐야')에는 personal_list_schedules와 "
            "personal_list_saved_schedules를 쓰지 않는다. 앞은 현재 대화의 임시 메모리만 보고 뒤는 개인 "
            "일정만 보기 때문에, 둘 다 같은 날의 그룹 일정·할 일·알림을 놓친다. "
            "대신 list_saved_requests에 date_from/date_to만 넘기고 kind는 비워 저장기록 전체를 한 번에 조회한다. "
            "사용자가 종류를 분명히 말했을 때만 kind 필터를 넘기고, 제목·내용의 키워드로 찾아야 하면 "
            "search_saved_requests를 쓴다. personal_list_saved_schedules는 사용자가 '개인 일정만' 같이 "
            "범위를 직접 좁혔을 때만 쓴다. "
