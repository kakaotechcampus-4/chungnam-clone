...
def add_personal_reference_dict(
    ...
    *,
    ...
    normalized_tags = tags if tags is not None else []
    reference = reference_store.add_personal_reference(title=title, content=content, tags=normalized_tags)
    return {
...
def search_saved_request_rows(
    ...
    *,
    ...
    return sqlite_store.search_saved_requests(query, limit=top_k) or []
...
def search_conversation_messages_dict(
    ...
    *,
    ...
    return {
        "hits": hits,
        "rows": hits,
        "context": conversation_rag_store.context_from_hits(conversation_hits),
...
def search_personal_references(query: str, top_k: int = 2) -> str:
    ...
    hits = search_personal_reference_hits(REFERENCE_STORE, query=query, top_k=normalized_top_k)
...
def search_saved_requests(query: str, top_k: int = 3) -> str:
    ...
    rows = search_saved_request_rows(SQLITE_STORE, query=query, top_k=normalized_top_k)
...
def search_nana_memory(
    ...
    candidate_rows = search_saved_request_rows(
        SQLITE_STORE, query=query, top_k=max(normalized_limit * 3, normalized_limit)
    )
    normalized_attendee = (attendee or "").strip().lower()
    ...
    for row in candidate_rows:
        ...
        if normalized_attendee:
            members = [str(member).lower() for member in _decode_attendees(row.get("members_json"))]
            if not any(normalized_attendee in member for member in members):
                continue
    ...
    if reference_hits:
        for hit in reference_hits:
            ...
            context_lines.append(f"- {metadata.get('title') or '제목 없음'}: {hit.get('content') or ''}")
    else:
    ...
    if schedule_rows:
        for row in schedule_rows:
            ...
            context_lines.append(
                f"- {row.get('title') or '제목 없음'} | {row.get('date') or '날짜 미정'} "
                f"{row.get('start_time') or ''} | 참석자={members}"
            )
    else:
        context_lines.append("- 관련 저장 기록이 없습니다.")
...
def week04_prompt_parts() -> list[str]:
    ...
    return [
        *week03_prompt_parts(),
        (
            f"오늘 날짜는 {today_iso}입니다. Week 4부터 Nana는 출처가 서로 다른 세 가지 검색 도구를 "
            "구분해서 사용합니다. 질문의 성격에 맞는 도구를 하나 또는 여러 개 선택해서 호출하고, "
            "근거가 부족하면 추측하지 말고 도구를 더 호출해 확인하세요.\n"
            f"- {search_personal_references.name}: 사용자가 직접 등록한 개인 참고자료(선호, 규칙, 메모)를 "
            "ChromaDB + OpenAI embedding으로 검색합니다. 회의 선호, 팀 규칙처럼 배경 지식/취향을 물을 때 사용하세요.\n"
            f"- {search_saved_requests.name}: 이 앱이 SQLite에 저장한 구조화된 일정/할 일/알림 기록"
            "(structured_requests/schedules)을 검색합니다. 저장해 둔 일정/할 일/알림 같은 구조화된 사실을 "
            "조회할 때 사용하세요.\n"
            f"- {search_conversation_messages.name}: 앱 SQLite에 남아 있는 일반 채팅 발화를 대화 단위로 "
            "ChromaDB에 lazy sync한 뒤 검색합니다. 예전 대화에서 무슨 이야기를 했는지 회상할 때 사용하세요. "
            f"단, {search_conversation_messages.name} 결과에서 role이 assistant인 발화는 Nana 자신의 이전 "
            "답변일 뿐이므로, 그 발화 내용만으로 사실을 단정하지 말고 필요하면 다른 도구로 다시 확인하세요."
        ),
    ]
