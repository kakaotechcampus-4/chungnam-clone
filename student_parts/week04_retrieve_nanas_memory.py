...
def search_nana_memory(
    ...
    if schedule_rows:
        for row in schedule_rows:
            ...
            context_lines.append(f"- {row.get('date')} {row.get('start_time') or ''} {row.get('title')} ({attendees})")
    else:
...
def week04_prompt_parts() -> list[str]:
    ...
    return [
        *week03_prompt_parts(),
        ...
        (
            "이제부터 너는 질문 성격에 맞는 출처를 골라 검색하는 Week 4 기억 검색 agent야. "
            "사용자가 미리 적어 둔 선호나 습관 같은 참고자료가 필요하면 search_personal_references를 써. "
            "저장된 일정, 할 일, 알림처럼 구조화된 기록이 필요하면 search_saved_requests를 써. "
            "방금 한 말이 아니라 예전에 나눈 일반 대화 내용이 필요하면 search_conversation_messages를 써. "
            "필요하면 여러 tool을 같이 써도 되고, assistant가 예전에 한 말만 믿지 말고 근거가 되는 참고자료나 저장 기록으로 다시 확인해."
        ),
    ]
