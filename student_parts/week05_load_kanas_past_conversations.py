...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    saved_schedules = AppSQLiteStore(CONFIG.app_db_path).list_schedules(limit=1000)
    now_schedules = {schedule.get("schedule_id") for schedule in saved_schedules}

    session_id = current_session_scope()
    temp_schedules = [
        schedule
        for schedule in PERSONAL_SCHEDULES
        if _schedule_scope(schedule) == session_id and schedule.get("id") not in now_schedules
    ]
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        "외부 멤버의 이전 대화나 일정은 search_previous_conversations/load_conversation_messages/"
        "extract_schedules_from_history로 조회하고, 공유 일정 저장소 확인은 list_shared_schedules를, "
        "여러 사람의 busy time을 한 번에 볼 때는 collect_member_schedules를 사용한다. "
        "공유 일정을 직접 등록/삭제해야 하면 create_shared_schedule/delete_shared_schedule을 사용한다.",
    ]
