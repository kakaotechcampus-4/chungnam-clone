...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    sqlite_schedules = AppSQLiteStore(CONFIG.app_db_path).list_schedules(limit=1000)
    sqlite_ids = {row.get("schedule_id") for row in sqlite_schedules}

    current_scope = current_session_scope()
    temp_schedules = [
        schedule
        for schedule in PERSONAL_SCHEDULES
        if _schedule_scope(schedule) == current_scope and schedule.get("id") not in sqlite_ids
    ]
...
def load_conversation_messages(conversation_id: str) -> str:
    ...
    result_dict = call_external_tool_payload("load_conversation_messages", {"conversation_id": conversation_id})
