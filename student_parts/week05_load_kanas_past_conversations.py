...
def resolve_external_member_names(
    member_names: list[str] | None,
) -> list[str] | None:
    """전체 외부 멤버 표현을 fixture의 실제 멤버 이름 목록으로 확장합니다."""

    if member_names is None:
        return None

    normalized_labels = {
        "".join(str(member_name).split())
        for member_name in member_names
        if str(member_name).strip()
    }
    if normalized_labels & _ALL_EXTERNAL_MEMBER_LABELS:
        return list(JULY_PRACTICE_MEMBER_NAMES)
...
def search_previous_conversations(
    ...
    args = {
        "query": query,
        "member_names": resolve_external_member_names(member_names),
        "limit": limit,
    }
...
def collect_member_schedules(
    ...
    payload = _collect_member_schedules(
        member_names=resolve_external_member_names(member_names) or [],
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        전체를 뜻하는 표현을 사용하면 철수, 영희, 민준, 서연, 지훈, 하린 전원을
        member_names에 전달한다. 이 표현 자체를 사람 이름으로 전달하지 않는다.
