...
- **과제가 회차(1회차/2회차 등)로 나뉘어 있으면 진행 구조를 여기서 먼저 정한다 (4주차 반성).**
  기본값은 **"계획은 하나만 공유 · 구현과 커밋만 회차별로 분리 · 동작 eval(통과율)은 파일이 온전해지는 마지막 회차에서"** 다.
  회차마다 planner를 다시 돌리지 않는다(두 독립 판단이 붕괴해 교차검증이 사라진다). 앞 회차만으로 동작 baseline을
  잡지 않는다(라우팅 프롬프트가 뒤 회차 산출물이면 앞 회차 단독 기준값은 반쪽이다). 이 구조 결정은 실행 전에
  내리는 것이라 eval/trace가 못 잡는다 → **게이트에서 명시적으로 확인**한다. 앞 회차 계약 검증(Step 2)은 해당 회차
  부분집합만 판정하고, 뒤 회차 미구현으로 인한 실패는 N/A(범위 밖)로 남긴다.
- → **[게이트] 계획 승인**
...
- **앱 직접 실행(`./run.sh --weekNN`)은 선택이 아니라 필수 단계다 (4주차 반성).** verify·eval이 다 통과한 뒤에도
  반드시 앱을 띄워 trace를 눈으로 본다 — 4주차의 진짜 결함 2건(추가 경로 오연결·출처 혼동)은 전부 여기서 나왔다.
  회귀 확인용이 아니라 **새 실패 모드 탐색용**이며, 발견하면 손으로 반복 확인하지 말고 **eval 케이스로 승격 → 측정 → 수정 → 재측정**한다.
  단, 범위 밖 발견(앞 주차 성향 등)은 분류만 하고 고치지 않는다.
