...
> `plan_modify.md`(PR 코드 리뷰 피드백을 반영하기 위한 리팩토링 계획서)의 마일스톤 R1~R3를 순서대로
> 반영한 기록. 대상 파일은 `student_parts/week03_build_nanas_logbook.py` 하나이며, 다른 파일은
> 손대지 않았다.

### R1 — 프롬프트 압축 (Prompt DRY)

**리뷰 피드백 요지**: `WEEK03_TOOL_CALL_PROMPT`와 `week03_prompt_parts()`에서 반복되는
`extract_schedule_request → save_structured_request` 구조화 연쇄 지시 및 날짜 필터 제거 지침을
간결한 단일 지시문으로 압축해 토큰 비용을 줄이고 캐싱 효율을 높인다.

**발견한 중복**:
- "자연어 생성엔 extract_schedule_request→save_structured_request만 쓰고 personal_create_schedule은
  안 쓴다"는 규칙이 **여는 문단 + 옛 규칙(1)(2) + 옛 규칙(9) + 닫는 문단**, 총 4곳에서 거의 같은
  내용으로 반복되고 있었다.
- "날짜 미지정 시 전체 기간 조회"가 **옛 규칙(4) 임베디드 + 옛 규칙(8) + "기준일" 디스클레이머**,
  3곳에서 반복되고 있었다.

**조치**: 저장 경로 단일화 규칙은 `WEEK03_TOOL_CALL_PROMPT` 새 규칙(1) **한 곳에만** 배치하되, 기존에
효과가 검증된 강한 문체("이것은 반드시 지켜야 하는 규칙이다")는 그대로 유지했다. 날짜 스코프 규칙도
새 규칙(3) 하나로 합치되, 수정/삭제 후보 확인 호출부(옛 규칙 4의 위치) 바로 옆에 남겨 로컬리티를
지켰다(6-6에서 "위치가 준수율에 영향을 준다"는 게 실증됐으므로). `week03_prompt_parts()`의 여는 문단/
닫는 문단은 전체 재진술 대신 "아래 tool 호출 규칙을 따른다"는 짧은 상호 참조로 축약했다. 규칙 번호를
10개→7개로 재정렬했다(옛 1·2·9→새 1, 옛 3→새 2, 옛 4·8→새 3, 옛 5→새 4, 옛 6→새 5, 옛 7→새 6, 옛 10→새 7).

또한 새 규칙(4)의 문구를 R3(삭제 가드 강화)의 코드 변경과 맞춰, "schedule_ids가 없으면 date/title/
start_time/time_unspecified 중 최소 2개를 함께 지정해야 한다"로 갱신해 프롬프트와 코드가 어긋나지
않게 했다(이렇게 안 하면 LLM이 단일 필터로 삭제를 시도했다가 매번 거부당하는 낭비가 생긴다).

**위험 관리 및 검증**: 압축이 기존에 어렵게 확립한 신뢰성을 깨지 않는지가 가장 중요했으므로,
실제 LLM으로 기존 회귀 시나리오를 전부 재실행했다.
- **6-5(중복 저장 방지)**: `"내일 19시에 초은이 누나랑 카리코에서 데이트 약속 잡아줘"`류 문장을
  **5회 반복 → 5/5 전부** 정확히 한 경로만 호출, DB에 1건만 저장.
- **6-6(수정 요청 시 날짜 스코프)**: `"OO를 오후 3시로 바꿔줘"`(날짜 미언급)를 **6회 반복 → 6/6 전부**
  `date_from`/`date_to`가 `None`으로 유지.
- **규칙 6(후보 다건 시 되묻기)**: 동일 제목 2건 상태에서 수정 요청 → `personal_update_saved_schedule`
  미호출, 되묻는 답변 확인.
- **규칙 7(group_schedule 폴백)**: "동아리 회식" 저장 후 삭제 요청 → 정상적으로 찾아 삭제, 남은 행 0건.
- **10단계 통합 시나리오**(생성→확인→생성→확인→수정→확인→삭제→확인→삭제→확인(0개)) 1세트 전체 재실행
  → 이슈 없음, 최종 DB 0건.
